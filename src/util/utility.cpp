...
bool curl_download(const char *url, const char *path)
{
	...
	if (fp && curl) {
		...
		if (res != CURLE_OK) {
			blog(LOG_ERROR,
			     "[tuna] Couldn't fetch file from %s to %s", url,
			     path);
		}
#ifdef DEBUG
		else {
...
void handle_cover_art(const song_t *song)
{
	...
	if (found_cover && !song->cover.empty() && song->cover != last_cover) {
		if (is_url && config::download_cover) {
			...
			found_cover =
				curl_download(song->cover.c_str(), tmp.c_str());
			...
			last_cover = song->cover;
			found_cover = move_file(song->cover.c_str(),
						config::cover_path);
		}
	}
	...
	if (!found_cover && last_cover != "n/a") {
		...
		if (!move_file(config::cover_placeholder, config::cover_path))
			blog(LOG_ERROR,
			     "[tuna] couldn't move placeholder cover");
	}
}

void replace_all(std::string &str, const std::string &find,
