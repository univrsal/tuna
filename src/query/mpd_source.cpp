...
{
	m_capabilities =
			CAP_TITLE | CAP_ALBUM | CAP_PROGRESS | CAP_VOLUME_UP | CAP_VOLUME_DOWN | CAP_VOLUME_MUTE | CAP_LENGTH |
			CAP_PLAY_PAUSE | CAP_NEXT_SONG | CAP_PREV_SONG;
	m_address = nullptr;
	m_port = 0;
	m_connected = false;
}
...
void mpd_source::connect()
{
	...
	if (mpd_connection_get_error(m_connection) != MPD_ERROR_SUCCESS) {
		blog(LOG_ERROR, "[tuna] mpd connection to %s:%hu failed with error %s", m_address, m_port,
